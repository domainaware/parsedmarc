...
class TestParserConfigCaches(unittest.TestCase):
    ...
    def test_each_instance_gets_isolated_caches(self):
        ...
        self.assertIsNot(cfg_a.ip_address_cache, parsedmarc.config.IP_ADDRESS_CACHE)
        self.assertIsNot(
            cfg_a.seen_aggregate_report_ids,
            parsedmarc.config.SEEN_AGGREGATE_REPORT_IDS,
        )
        self.assertIsNot(cfg_a.reverse_dns_map, parsedmarc.config.REVERSE_DNS_MAP)

        self.assertIsNot(cfg_b.ip_address_cache, parsedmarc.config.IP_ADDRESS_CACHE)
        self.assertIsNot(
            cfg_b.seen_aggregate_report_ids,
            parsedmarc.config.SEEN_AGGREGATE_REPORT_IDS,
        )
        self.assertIsNot(cfg_b.reverse_dns_map, parsedmarc.config.REVERSE_DNS_MAP)
        ...
        self.assertNotIn("1.2.3.4", parsedmarc.config.IP_ADDRESS_CACHE)
        self.assertNotIn("report-id-a", parsedmarc.config.SEEN_AGGREGATE_REPORT_IDS)
        self.assertNotIn("example.com", parsedmarc.config.REVERSE_DNS_MAP)
    ...
    def test_module_default_caches_are_the_public_globals(self):
        ...
        ParserConfig.
        """
        self.assertIs(parsedmarc.IP_ADDRESS_CACHE, parsedmarc.config.IP_ADDRESS_CACHE)
        self.assertIs(
            parsedmarc.SEEN_AGGREGATE_REPORT_IDS,
            parsedmarc.config.SEEN_AGGREGATE_REPORT_IDS,
        )
        self.assertIs(parsedmarc.REVERSE_DNS_MAP, parsedmarc.config.REVERSE_DNS_MAP)
...
class TestParserConfigPickling(unittest.TestCase):
    ...
    def test_pickle_drops_cache_contents_and_binds_process_defaults(self):
        ...
        cfg = ParserConfig(dns_timeout=9.5, nameservers=["192.0.2.53"])
        ...
        self.assertIs(restored.ip_address_cache, parsedmarc.config.IP_ADDRESS_CACHE)
        self.assertIs(
            restored.seen_aggregate_report_ids,
            parsedmarc.config.SEEN_AGGREGATE_REPORT_IDS,
        )
        self.assertIs(restored.reverse_dns_map, parsedmarc.config.REVERSE_DNS_MAP)

        # Sentinel entries placed on the original instance's caches must NOT
        # have crossed into the module defaults.
        self.assertNotIn("sentinel-ip", parsedmarc.config.IP_ADDRESS_CACHE)
        self.assertNotIn(
            "sentinel-report-id", parsedmarc.config.SEEN_AGGREGATE_REPORT_IDS
        )
        self.assertNotIn("sentinel.example", parsedmarc.config.REVERSE_DNS_MAP)
