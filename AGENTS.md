...
Three more rules, from the PR #858 review cycle (Copilot and Codecov each caught defects that both the author's implementation-time reviews and a fresh-context diff review had passed over):

- **A plain-type docstring is wrong when `None` is a semantic state.** This codebase's docstring convention documents optional parameters with bare types (`ip_db_path (str)` for `str | None`), which is fine while `None` merely means "not provided". On #858, four new kwargs used `None` as the inheritance mechanism (`None` = inherit `delete`; explicit `False` = opt out) — a meaningful third state — and every reviewer pattern-matched the `(bool)` entries as conventional. Read docstrings as a naive library caller: if the docs never say that `None`, not `False`, is how the default behavior is selected, the docstring is wrong no matter how conventional it looks. When a parameter's `None` is load-bearing, document the union type and the sentinel's meaning explicitly.
- **"Pre-existing" is valid triage only while the diff doesn't extend what the defect ranges over.** The `config=` docstring paragraph claimed the "keyword arguments listed above are ignored" when `config=` is provided — already loosely false for `delete`/`test`/`batch_size`, so the implementation-time review filed it as pre-existing and out of scope, and the later review passes accepted that triage without re-deriving it. But the PR added four entries to the very list that universal claim quantifies over, changing the claim's truth value for the new arguments — for them it was not pre-existing at all. A quantified claim is an enumeration in disguise (see the #851 counting rule above): when a diff extends the set a nearby claim ranges over, the claim is part of the diff, and "pre-existing" stops being a reason to skip it.
- **Verify CI's gates, not just CI's commands — and a check that matches nothing is broken, not green.** The #851 rule above ("run CI's literal commands") was followed and still wasn't enough: Codecov's patch-coverage gate corresponds to no command in the workflow file, so nobody asked "does a test execute every new line?", and the delete-error handler in a rewritten disposal loop shipped uncovered. The loop was a log-equivalent rewrite of old code, but rewritten lines are new patch lines even when behavior is intentionally identical — "moved code is new code" applies to coverage, not just correctness. Before opening a PR, compare coverage's missing-lines set against the diff (`pytest --cov --cov-report=term-missing`, or diff `coverage.xml` against the patch). And build such ad hoc checks to fail loudly on empty matches: the author's post hoc script filtered `coverage.xml` on `parsedmarc/__init__.py` while the report stores source-relative paths (`__init__.py`), matched nothing, and printed an empty "all covered" result that was read as green. Silence is not success.
